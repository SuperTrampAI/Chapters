...
# 使用多线程的优势与劣势
# 无论是多线程还是多进程，在多个之前切换，效率上不去
# 第二个考虑的是，任务的类型，可以把任务分为计算密集型和IO密集型

# 单线程+异步IO
# 利用异步IO的支持，可以用但竟成单线程模型来执行多任务，这种模型成为事件驱动模型。
# 在python中，单线程+异步IO的编程模型称为协程
# 在使用c端按钮时，对于按钮以后的触发事件采用多线程的方式。
